    ...
    public function run()
    {
		...
		for ($i=1;$i<=6;$i++):
		     DB::table('arbitros')->insert([
				 'nombre_arbitro'=>$faker->name(),
				 'apellidos_arbitro'=>$faker->lastName(),
				 'nacionalidad_arbitro'=>$faker->unique()->randomElement(self::$arrayPais),
				 'email_arbitro'=>$faker->email,	
				 'telefono_arbitro'=>$faker->phoneNumber,
			]);
	    endfor;
    }
}